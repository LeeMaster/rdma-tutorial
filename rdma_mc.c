...
// Muticast example using RDMA_CM and IBV Verbs. And API definitions can refer
// to rdma/rdma_cma.h and infiniband/verbs.h
...
static int verify_test_params(struct cmatest_node *node) {
    ...
    ret =
        ibv_query_port(node->cma_id->verbs, node->cma_id->port_num, &port_attr);
...
static void destroy_node(struct cmatest_node *node) {
    ...
    // destroy the RDMA ID after all device resouces detoried
...
// 1. Get source (if provided for binding) and destination addreses - convert
//    the input addresses to socket presetation.
// 2. Joining:
//    A. For all connections:
//       - if sources addres is specifically provided, then bind the rdma_cm
//         object to the corresponding network interface. (Associates a src.
//         addres with an rdma_cm identifier).
//       - if unmapped MC address with bind address provided, check the remote
//         address and then bind.
//    B. Poll on all the connection events and wait that all rdma_cm objects
//       joined the MC group.
// 3. Send & receive:
//    A. If sender: send the messages to all connection nodes (function
//       "post_sends").
//    B. If receiver: poll the CQ (function "poll_cqs") till messages arrival.
// On ending - release network resources (per all connections: leaves the MC
// group and detaches its associated QP from the group)
static int run(void) {
    int i, ret;
    ...
    // Pause to give SM change to configure switches. We don't want to handle
    // reliability issue in this simple test program.
    sleep(3);
...
void print_usage(const char *progname) {
    ...
    printf("\t-M    unmapped_multicast_address, replaces -m and requires -b\n");
    printf("\t-s    sender\n");
...
// 3. Allocate Node and creates and identifier that is used to track comm. info.
