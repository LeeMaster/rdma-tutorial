...
void *client_thread_func(void *arg) {
    ...
    int msg_size = g_config_info.msg_size;
    int num_concurr_msgs = g_config_info.num_concurr_msgs;
    ...
    int num_wc = 20;
    struct ibv_qp **qp = g_ib_res.qp;
    struct ibv_cq *cq = g_ib_res.cq;
    struct ibv_srq *srq = g_ib_res.srq;
    struct ibv_wc *wc = NULL;
    uint32_t lkey = g_ib_res.mr->lkey;

    char *buf_ptr = g_ib_res.ib_buf;
    char *buf_base = g_ib_res.ib_buf;
    int buf_offset = 0;
    size_t buf_size = g_ib_res.ib_buf_size;
    ...
    for (i = 0; i < num_peers; i++) {
        for (j = 0; j < num_concurr_msgs; j++) {
            ret =
                post_srq_recv(msg_size, lkey, (uint64_t)buf_ptr, srq, buf_ptr);
    ...
    while (start_sending != true) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (wc[i].opcode == IBV_WC_RECV) {
                /* post a receive */
                post_srq_recv(msg_size, lkey, wc[i].wr_id, srq,
                              (char *)wc[i].wr_id);
    ...
    for (i = 0; i < num_peers; i++) {
        for (j = 0; j < num_concurr_msgs; j++) {
            ret = post_send(msg_size, lkey, (uint64_t)buf_ptr, (uint32_t)i,
                            qp[i], buf_ptr);
        ...
        for (i = 0; i < n; i++) {
            if (wc[i].status != IBV_WC_SUCCESS) {
                if (wc[i].opcode == IBV_WC_SEND) {
                    check(
                        0,
                        "thread[%ld]: send failed status: %s; wr_id = %" PRIx64
                        "",
                    ...
                    check(
                        0,
                        "thread[%ld]: recv failed status: %s; wr_id = %" PRIx64
                        "",
                        thread_id, ibv_wc_status_str(wc[i].status),
                        wc[i].wr_id);
                }
            ...
            if (wc[i].opcode == IBV_WC_RECV) {
                ...
                if (imm_data == MSG_CTL_STOP) {
                    ...
                    post_send(msg_size, lkey, 0, imm_data, qp[imm_data],
                              msg_ptr);
                }
