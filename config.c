...
struct config_info g_config_info;
...
int get_rank() {
    int ret = 0;
    uint32_t i = 0;
    uint32_t num_servers = g_config_info.num_servers;
    uint32_t num_clients = g_config_info.num_clients;
    ...
    g_config_info.rank = -1;
    for (i = 0; i < num_servers; i++) {
        if (strstr(hostname, g_config_info.servers[i])) {
            g_config_info.rank = i;
            g_config_info.is_server = true;
            break;
        }
    }

    for (i = 0; i < num_clients; i++) {
        if (strstr(hostname, g_config_info.clients[i])) {
            if (g_config_info.rank == -1) {
                g_config_info.rank = i;
                g_config_info.is_server = false;
                break;
            } else {
                check(0, "node (%s) listed as both server and client",
                      hostname);
            }
        }
    }

    check(g_config_info.rank >= 0, "Failed to get rank for node: %s", hostname);
...
int parse_config_file(char *fname) {
    ...
    while (fgets(line, 128, fp) != NULL) {
        ...
        if (attr == ATTR_SERVERS) {
            ret = parse_node_list(line, &g_config_info.servers);
            check(ret > 0, "Failed to get server list");
            g_config_info.num_servers = ret;
        } else if (attr == ATTR_CLIENTS) {
            ret = parse_node_list(line, &g_config_info.clients);
            check(ret > 0, "Failed to get client list");
            g_config_info.num_clients = ret;
        } else if (attr == ATTR_MSG_SIZE) {
            g_config_info.msg_size = atoi(line);
            check(g_config_info.msg_size > 0, "Invalid Value: msg_size = %d",
                  g_config_info.msg_size);
        } else if (attr == ATTR_NUM_CONCURR_MSGS) {
            g_config_info.num_concurr_msgs = atoi(line);
            check(g_config_info.num_concurr_msgs > 0,
                  "Invalid Value: num_concurr_msgs = %d",
                  g_config_info.num_concurr_msgs);
        }
...
void destroy_config_info() {
    int num_servers = g_config_info.num_servers;
    int num_clients = g_config_info.num_clients;
    int i;

    if (g_config_info.servers != NULL) {
        for (i = 0; i < num_servers; i++) {
            if (g_config_info.servers[i] != NULL) {
                free(g_config_info.servers[i]);
            }
        }
        free(g_config_info.servers);
    }

    if (g_config_info.clients != NULL) {
        for (i = 0; i < num_clients; i++) {
            if (g_config_info.clients[i] != NULL) {
                free(g_config_info.clients[i]);
            }
        }
        free(g_config_info.clients);
    }
}
...
void print_config_info() {
    ...
    log("rank                      = %d", g_config_info.rank);
    log("msg_size                  = %d", g_config_info.msg_size);
    log("num_concurr_msgs          = %d", g_config_info.num_concurr_msgs);
    log("sock_port                 = %s", g_config_info.sock_port);
